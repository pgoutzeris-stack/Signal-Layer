-- Die Startprognose nimmt die letzten 15 ähnlichen Läufe (nicht 40), zählt
-- Reparatur-Schreiben zur Modellzeit und misst Motive an image_start/images_done.
...
begin
  with pool as (
    ...
    where status = 'done'
      ...
      and (p_images is null or coalesce(nullif(answers->>'images', ''), 'auto') = p_images)
      and (
        p_benchmarks_mode is null
        or coalesce(nullif(answers->>'benchmarks_mode', ''), nullif(answers->>'benchmarks', ''), 'auto') = p_benchmarks_mode
      )
  ...
  segs as (
    select
      ...
      greatest(
        200,
        coalesce(lead((e->>'t')::int) over (partition by p.id order by (e->>'t')::int), p.duration_ms) - (e->>'t')::int
      ) as dur
    from pool p
  ...
  per as (
    select id, stage,
      case when stage = 'modell' then sum(dur) else max(dur) end as dur
    from segs
  ...
  img as (
    select
      (select max((e->>'t')::int) from jsonb_array_elements(p.run_log) e where e->>'event' = 'images_done')
      - (select min((e->>'t')::int) from jsonb_array_elements(p.run_log) e where e->>'event' = 'image_start') as dur
    from pool p
    where exists (select 1 from jsonb_array_elements(p.run_log) e where e->>'event' = 'image_start')
      and exists (select 1 from jsonb_array_elements(p.run_log) e where e->>'event' = 'images_done')
  ),
  stage_med as (
    select stage, round(percentile_cont(0.75) within group (order by dur))::integer as ms
    from (
      select stage, dur from per where stage <> 'bilder'
      union all
      select 'bilder', dur from img where dur between 2000 and 900000
    ) x
    group by stage
    having count(*) >= 1
  )
  select
    jsonb_build_object(
      ...
      'scope', 'exact',
      'stages', coalesce((select jsonb_object_agg(stage, ms) from stage_med), '{}'::jsonb)
    )
  into result
  from totals;
...
comment on function signal_layer.asset_duration_forecast(text, text, integer, text, text) is
  'p75 der letzten 15 ähnlichen Läufe; Modellzeit inkl. Reparatur; Motive an image_start/images_done.';
