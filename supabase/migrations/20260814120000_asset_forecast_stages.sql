-- Stufenmediane für die Restzeit-Anzeige. Dieselbe Art, dasselbe Format,
-- bei Memos zusätzlich Bilder und Vorreiter-Modus.

drop function if exists signal_layer.asset_duration_forecast(text, text, integer);

create function signal_layer.asset_duration_forecast(
  p_kind text,
...
begin
  ...
  stage_med as (
    ...
    from stage_rows
    where dur between 200 and 300000
    group by stage
    having count(*) >= 1
  )
  select
...
comment on function signal_layer.asset_duration_forecast(text, text, integer, text, text) is
  'Median-Dauer und Stufenzeiten ähnlicher erfolgreicher Assets für die Restzeit-Anzeige.';
